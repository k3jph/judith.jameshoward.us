...
if (slugs.length !== 58 || uniqueSlugs.size !== slugs.length) {
  throw new Error(`Collection audit failed: expected 58 unique object slugs, found ${uniqueSlugs.size}.`);
}
...
for (const mapName of ["madePlacements", "heldPlacements"]) {
  ...
  const intentionallyUnplaced = mapName === "madePlacements" ? ["cindy-sherman-untitled-228"] : [];
...
if (imageCount !== 43 || textOnlyCount !== 15) throw new Error(`Media audit failed: expected 43 images and 15 text-only records; found ${imageCount} and ${textOnlyCount}.`);
