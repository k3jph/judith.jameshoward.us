...
export default function ResearchPage() {
  return <PageShell className="research-page">
    ...
    <section className="research-ledger section-pad"><header className="section-heading"><div><p className="eyebrow">Open ledger</p><h2>What the wider sweep<br />made visible</h2></div><p>Rights restrictions no longer keep a verified work out of the gallery: fifteen objects now have designed text-only records with the best available external link. The ledger is reserved for leads whose identity or object data still need resolution; works promoted to the gallery leave it.</p></header><div>{researchLeads.map(([collection, object, note, url]) => <article key={`${collection}-${object}`}><p>{collection}</p><h3><a href={url} target="_blank" rel="noreferrer">{object} ↗</a></h3><span>{note}</span></article>)}</div></section>

    <section className="research-gaps section-pad"><header className="section-heading"><div><p className="eyebrow">The unfinished exhibition</p><h2>What the current sweep<br />still misses</h2></div><p>Fifty-eight objects are a working corpus, not a completeness claim. These are active research gaps—not categories presumed unimportant because they are difficult to catalogue.</p></header><ol>{researchGaps.map(([title, note], index) => <li key={title}><span>{String(index + 1).padStart(2, "0")}</span><div><h3>{title}</h3><p>{note}</p></div></li>)}</ol></section>
